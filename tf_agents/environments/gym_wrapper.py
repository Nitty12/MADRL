...
def spec_from_gym_space(space,
                        ...
                        simplify_box_bounds=True,
                        name=None,
                        get_total_observation_spec = None,
                        get_total_action_spec = None):
  ...
  """Nitty: modifying to create obs_spec for using centralized critic with obs of all agents"""
  if get_total_observation_spec:
    if isinstance(space, gym.spaces.Tuple):
      if hasattr(space.spaces[0], 'dtype') and gym.spaces.Box not in dtype_map:
        dtype = space.spaces[0].dtype
      else:
        dtype = dtype_map.get(gym.spaces.Box, np.float32)
      minimum = np.asarray(space.spaces[0].low, dtype=dtype)
      maximum = np.asarray(space.spaces[0].high, dtype=dtype)
      ...
      for s in space.spaces:
        total_obs += s.shape[0]
      return specs.BoundedArraySpec(shape=(total_obs, ),
                                    dtype=dtype,
                                    minimum=minimum,
                                    maximum=maximum,
                                    name=name)

  """Nitty: modifying to create act_spec for using centralized critic with act of all agents"""
  if get_total_action_spec:
    if isinstance(space, gym.spaces.Tuple):
      if hasattr(space.spaces[0], 'dtype') and gym.spaces.Box not in dtype_map:
        dtype = space.spaces[0].dtype
      else:
      ...
      for i,s in enumerate(space.spaces):
        if i > 0:
          minimum = np.hstack((minimum, np.asarray(s.low, dtype=dtype)))
          maximum = np.hstack((maximum, np.asarray(s.high, dtype=dtype)))
        total_act += s.shape[0]
      return specs.BoundedArraySpec(shape=(total_act, ),
                                    dtype=dtype,
                                    minimum=minimum,
                                    maximum=maximum,
                                    name=name)
...
class GymWrapper(py_environment.PyEnvironment):
  ...
  def __init__(self,
    ...
    """Nitty: modifying to create obs_spec for using centralized critic with obs of all agents"""
    self._flat_tot_obs_spec = tf.nest.flatten(self._total_observation_spec)
    self._flat_tot_act_spec = tf.nest.flatten(self._total_action_spec)
  ...
  def _step(self, action):
    ...
    """Nitty: modification to prevent reset after every step"""
    if self._auto_reset and np.any(self._done):
      return self.reset()
