...
class CriticNetwork(network.Network):
  ...
  def call(self, inputs, step_type=(), network_state=(), training=False):
    ...
    if not hasattr(observations, 'shape'):
        """Nitty: while initializing, a random observation is given which is as per input_tensor_spec
                    This will have shape as it will be just a tensor
                    While running it will be tuple of tensors which dont have shape 
        critic network only accepts 1 observation and 1 action, so flattening the tuple of obs and action"""
        flat_obs = observations[0]
        for i, obs in enumerate(observations):
            if i > 0:
                flat_obs = tf.concat([flat_obs, obs], 1)
        flat_act = actions[0]
        for i, act in enumerate(actions):
            if i > 0:
                flat_act = tf.concat([flat_act, act], 1)

        del step_type  # unused.
        flat_obs = tf.cast(tf.nest.flatten(flat_obs)[0], tf.float32)
        for layer in self._observation_layers:
          flat_obs = layer(flat_obs, training=training)

        flat_act = tf.cast(tf.nest.flatten(flat_act)[0], tf.float32)
        for layer in self._action_layers:
          flat_act = layer(flat_act, training=training)

        joint = tf.concat([flat_obs, flat_act], 1)
