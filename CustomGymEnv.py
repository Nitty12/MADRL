...
class CustomGymEnv(gym.Env):
    ...
    def _set_action(self, action, agent, action_space, spotState):
        ...
        """
        if spotState:
            agent.spotBidMultiplier = action[:self.SpotMarket.dailySpotTime]
            # we dont use the price multiplier in spot market

        else:
            agent.flexBidMultiplier = action[self.SpotMarket.dailySpotTime:2*self.SpotMarket.dailySpotTime]
            agent.flexBidPriceMultiplier = action[2*self.SpotMarket.dailySpotTime:]
